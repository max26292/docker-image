...
COPY ./config/rsync/entrypoint.sh /entrypoint.sh

RUN mkdir -p /docker-entrypoint.d \
 && chmod +x /entrypoint.sh \
 && mkdir -p /etc/supervisor.conf.d

COPY ./config/rsync/supervisord.conf /etc/supervisord.conf
COPY ./config/rsync/supervisor.daemon.conf /etc/supervisor.conf.d/supervisor.daemon.conf
